...
mod tests {
    ...
    #[test]
    fn name() { assert_eq!(AlpacaBrokerageModel::default().name(), "Alpaca"); }

    #[test]
    fn default_leverage_positive() { assert!(AlpacaBrokerageModel::default().default_leverage() > 0.0); }

    #[test]
    fn can_submit() { assert!(AlpacaBrokerageModel::default().can_submit_order()); }

    #[test]
    fn fee_is_zero() {
        let fee = AlpacaBrokerageModel::default().transaction_model()
